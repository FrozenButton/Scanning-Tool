...
def capture_once() -> None:
    ...
    import threading
    if app_state.service_state.gui_status_callback:
        threading.Thread(target=_do_capture, args=(app_state.service_state.gui_status_callback,), daemon=True).start()
    else:
        _do_capture()
...
def continuous_scan_loop() -> None:
    ...
    while app_state.scan_state.continuous_mode:
        capture_once()
